...
class dataSet():
    ...
    def temp_load_data(self, dir='bnb.p'):
        ...
        self.train_image = data['train_image']
        self.train_label = data['train_label']
        self.test_image = data['test_image']
        self.test_label = data['test_label']
        self.train_size, self.width, self.height, self.color = data['train_image'].shape
        self.test_size = data['test_image'].shape[0]
        self.n_class = len(np.unique(self.train_label))

    def load_data(self, dir='image', test_dir=None): # 현경
        sys_file = []
    ...
    def Augmentation(self, color_aug): # 유리
        pass
